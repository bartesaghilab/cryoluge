...
def test_biquadratic():
    ...
    @parameter
    fn f(x: Coords[info], out fx: Value[info]):
        fx = rebind[Value[info]]( (x[0] - 3)*(x[1] + 4) + (x[0] - 5)**2 + (x[1] + 7)**2 - 9 )
        # NOTE: add some rotation to the paraboloid, so we make Powell's method actually
        #       search along the extra direction
    ...
    assert_equal_float[err_fn](rebind[Scalar[dtype]](result.x[0]), 7.66, eps=1e-2)
    assert_equal_float[err_fn](rebind[Scalar[dtype]](result.x[1]), -9.33, eps=1e-2)
    assert_equal_float[err_fn](rebind[Scalar[dtype]](result.fx), -21.33, eps=1e-2)
...
def test_minimizer():
    ...
    var result = PowellMinimizer[dtype](
        line_search = BrentLineSearch(
            max_iterations = 10,
            min_interval_width = 1e-2
        ),
